...
services:
  traefik:
    ...
    volumes:
      ...
      - /docker/config/traefik/plugin/geoblock:/plugins-local/src/github.com/linkphoenix/geoblock-with-geojs-io/
...
Legacy key `blacklist` is also supported for backward compatibility.
...
Allows returning a HTTP 302 status code (Found) and redirecting the client to the configured URL instead of directly blocking the request.
