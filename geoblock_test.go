...
import (
	...
	geoblock "github.com/linkphoenix/geoblock-with-geojs-io"
)
...
func TestNilConfig(t *testing.T) {
	...
	_, err := geoblock.New(ctx, next, nil, "GeoBlock")
	if err == nil {
		t.Fatal("nil config should return an error")
	}
...
func TestInvalidAllowedIPAddress(t *testing.T) {
	cfg := createTesterConfig()
	cfg.Countries = append(cfg.Countries, "CH")
	cfg.AllowedIPAddresses = append(cfg.AllowedIPAddresses, "invalid-ip")
	...
	_, err := geoblock.New(ctx, next, cfg, "GeoBlock")
	if err == nil {
		t.Fatal("invalid allowed IP address should return an error")
	}

	if !strings.Contains(err.Error(), "invalid allowed IP address provided") {
		t.Fatalf("unexpected error: %v", err)
	}
}
